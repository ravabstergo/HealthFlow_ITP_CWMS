...
import { Search, ChevronLeft, ChevronRight, Bell, HelpCircle, Settings, ChevronDown, AlertCircle } from "lucide-react"
import { getDoctorSchedules, deleteAvailability, createSchedule, checkAvailabilityUpdatable, updateAvailability } from "../services/doctorService"
...
export default function HealthFlowDashboard() {
  ...
  const handleUpdate = async (availabilityItem) => {
    try {
      ...
      const result = await checkAvailabilityUpdatable(doctorId, availabilityItem._id);
      ...
      if (result.updatable) {
        ...
        if (Array.isArray(schedules)) {
          ...
          for (const schedule of schedules) {
            const found = schedule.availability.some(a => a._id === availabilityItem._id);
            if (found) {
        ...
        showAlert("Cannot update this availability because it contains booked appointments", "error");
      }
    } catch (error) {
    ...
    try {
      ...
      if (error.response) {
        if (typeof error.response.data === 'object' && error.response.data.message) {
          if (error.response.data.message.includes('contains booked appointments')) {
            errorMessage = "This availability cannot be deleted because it contains slots that are already booked by patients.";
          } else {
      ...
      {alertMessage && (
        <div 
          className={`fixed top-4 right-4 p-4 rounded-md shadow-md flex items-center z-50 ${
            alertMessage.type === "error" ? "bg-red-50 text-red-800 border border-red-200" : 
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="border-b border-gray-200">
          <div className="flex px-4">
            <TabButton label="Calendar" active={activeTab === "Calendar"} onClick={() => handleTabChange("Calendar")} />
            <TabButton label="Schedule" active={activeTab === "Schedule"} onClick={() => handleTabChange("Schedule")} />
          </div>
        </div>
            ...
            <div className="flex items-center justify-between p-4 border-b">
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">Edit availability details</p>
                </div>
              </div>
              <button
                className="h-8 w-8 flex items-center justify-center rounded-lg hover:bg-gray-100"
