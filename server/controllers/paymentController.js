...
const generatePaymentHash = (orderId, amount, currency, merchantSecret) => {
    ...
    const stringToHash = process.env.PAYHERE_MERCHANT_ID + orderId + amountFormatted + currency + hashedSecret;
...
const validatePaymentNotification = (merchantId, orderId, payhere_amount, payhere_currency, status_code, merchantSecret, md5sig) => {
    ...
    const stringToHash = merchantId + orderId + payhere_amount + payhere_currency + status_code + hashedSecret;
    console.log('Validation string to hash:', stringToHash);
    const calculatedHash = crypto.createHash('md5').update(stringToHash).digest('hex').toUpperCase();
...
exports.initiatePayment = async (req, res) => {
    try {
        console.log('\n=== PAYMENT INITIATION ===');
        console.log('Payment initiation request received:', JSON.stringify(req.body, null, 2));
        ...
        console.log('MERCHANT_SECRET exists:', !!process.env.PAYHERE_MERCHANT_SECRET);
...
exports.handlePaymentSuccess = async (req, res) => {
    ...
    while (retryCount < MAX_RETRIES) {
        try {
            ...
            const existingAppointment = await Appointment.findOne({ orderId }).session(session);
            ...
            if (!appointmentDetails) {
                console.error('Appointment details not found in cache for order:', orderId);
                await session.abortTransaction();
                session.endSession();
                return res.status(404).json({
                    success: false,
                    message: 'Appointment details not found. The payment session may have expired.'
                });
            }
            ...
            const schedules = await DoctorSchedule.find({ doctorId: appointmentDetails.doctorId }).session(session);
            ...
            if (error.errorLabels?.includes('TransientTransactionError') && retryCount < MAX_RETRIES - 1) {
            ...
            return res.status(500).json({
                ...
                error: error.message,
                isTransient: error.errorLabels?.includes('TransientTransactionError') || false
            });
        }
    }
};
...
exports.handlePaymentNotification = async (req, res) => {
    ...
    try {
        ...
        if (status_code === '2') {
            ...
            const existingAppointment = await Appointment.findOne({ orderId: order_id }).session(session);
            
            if (!existingAppointment) {
                const appointmentDetails = await cacheUtil.getAppointmentDetails(order_id);
                if (appointmentDetails) {
        ...
        res.status(500).json({ message: 'Error handling payment notification', error: error.message });
    }
};